...
def isotropic_reduction():
    axis_x = np.linspace(0.0, 1.5, 17)
    axis_y = np.linspace(0.0, 0.75, 13)
    grid_x, grid_y = np.meshgrid(axis_x, axis_y)
    coefficient = np.exp(
        0.3 * np.sin(2.0 * np.pi * grid_x / 1.5)
        + 0.2 * np.cos(2.0 * np.pi * grid_y / 0.75)
    )
    ...
    return {
        "grid_shape": list(coefficient.shape),
        "max_abs_solution_difference": float(np.max(np.abs(diagonal - scalar))),
    }
...
def ratio_diagnostics():
    ...
    for ratio in (0.25, 1.0, 4.0, 16.0):
        ...
        actual_ratio = dataset.inputs[..., 0] / dataset.inputs[..., 1]
        records.append(
            {
                "anisotropy_ratio": ratio,
                "max_abs_ratio_error": float(np.max(np.abs(actual_ratio - ratio))),
...
def main():
    report = {
        ...
        "isotropic_reduction": isotropic_reduction(),
        "ratio_families": ratio_diagnostics(),
        "limitations": [
            "diagonal symmetric positive tensor only",
